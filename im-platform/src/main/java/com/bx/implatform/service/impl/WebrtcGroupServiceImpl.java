...
public class WebrtcGroupServiceImpl implements WebrtcGroupService {
    ...
    public void setup(WebrtcGroupSetupDTO dto) {
        ...
        // 发送文字提示信息
        String content = formatUserMark(mineInfo.getId(), mineInfo.getNickName()) + "发起了语音通话";
        sendTipMessage(dto.getGroupId(), content);
        // 推送通话信息，让会话顶部显示"xx人正在通话中"
        sendRTCInfo(dto.getGroupId());
        // 超时未接通检测
        detectTimeout(webrtcSession.getChatId(), dto.getGroupId(), hoster, recvIds);
    ...
    public void accept(Long groupId) {
        ...
        List<Long> inChatUserIds =
            webrtcSession.getInChatUsers().stream().map(IMUserInfo::getId).collect(Collectors.toList());
        sendRtcMessage1(MessageType.RTC_GROUP_ACCEPT, groupId, recvIds, JSON.toJSONString(inChatUserIds), true);
        // 停止离线呼叫
    ...
    public void join(Long groupId) {
        ...
        if (!Objects.isNull(mine) && !mine.getTerminal().equals(userSession.getTerminal())) {
            throw new GlobalException("您已在其他设备加入通话");
        }
        ...
        userInfo.setIsShareScreen(false);
        // 将当前用户加入通话用户列表中
        if (!isExist(webrtcSession, userSession.getUserId())) {
    ...
    public void forceQuit(Long groupId) {
        UserSession userSession = SessionContext.getSession();
        WebrtcGroupSession webrtcSession = getWebrtcSession(groupId);
        if (!isInchat(webrtcSession, userSession.getUserId(), userSession.getTerminal())) {
            return;
        }
    ...
    public void device(WebrtcGroupDeviceDTO dto) {
        ...
        userInfo.setIsShareScreen(dto.getIsShareScreen());
        saveWebrtcSession(dto.getGroupId(), webrtcSession);
        // 广播信令
        List<Long> recvIds = getRecvIds(webrtcSession.getUserInfos());
    ...
    private Boolean isInchat(WebrtcGroupSession webrtcSession, Long userId, Integer terminal) {
        return webrtcSession.getInChatUsers().stream()
            .anyMatch(user -> user.getId().equals(userId) && user.getTerminal().equals(terminal));
    }
    ...
    private void sendRTCInfo(Long groupId) {
        UserSession userSession = SessionContext.getSession();
        List<Long> userIds = groupMemberService.findUserIdsByGroupId(groupId);
        ...
        msg.setSendTime(new Date());
        msg.setStatus(MessageStatus.PENDING.code());
        msg.setType(MessageType.RTC_GROUP_INFO.code());
        // 群发
        GroupMessageVO msgInfo = BeanUtils.copyProperties(msg, GroupMessageVO.class);
        IMGroupMessage<GroupMessageVO> sendMessage = new IMGroupMessage<>();
        sendMessage.setSender(new IMUserInfo(userSession.getUserId(), userSession.getTerminal()));
    ...
    private void detectTimeout(Long chatId, Long groupId, IMUserInfo inviter, List<Long> userIds) {
