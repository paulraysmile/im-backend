...
public class FriendServiceImpl extends ServiceImpl<FriendMapper, Friend> implements FriendService {
    ...
    public void initDndCache(Long userId) {
        LambdaQueryWrapper<Friend> wrapper = Wrappers.lambdaQuery();
        wrapper.eq(Friend::getUserId, userId);
        wrapper.eq(Friend::getIsDnd, true);
        wrapper.select(Friend::getFriendId);
        List<Friend> friends = this.list(wrapper);
        Object[] ids = friends.stream().map(Friend::getFriendId).toArray();
        String key = StrUtil.join(":", RedisKey.IM_CACHE_FRIEND_DND, userId);
    ...
    public void setDnd(FriendDndDTO dto) {
        ...
        this.update(wrapper);
        // 更新缓存
        String key = StrUtil.join(":", RedisKey.IM_CACHE_FRIEND_DND, session.getUserId());
        if (dto.getIsDnd()) {
            redisTemplate.opsForSet().add(key, dto.getFriendId());
        } else {
    ...
    @Override
    public Boolean isDnd(Long userId, Long friendId) {
        String key = StrUtil.join(":", RedisKey.IM_CACHE_FRIEND_DND, userId);
        return redisTemplate.opsForSet().isMember(key, friendId);
    }
    ...
    private FriendVO convert(Friend f, List<IMTerminalType> onlineTerminals) {
        ...
        vo.setCompanyName(f.getFriendCompanyName());
        ...
        if (CollectionUtil.isNotEmpty(onlineTerminals)) {
            vo.setOnline(true);
            vo.setOnlineWeb(onlineTerminals.contains(IMTerminalType.WEB));
            vo.setOnlineApp(onlineTerminals.contains(IMTerminalType.APP));
        }
        return vo;
    }
    ...
    void sendAddFriendMessage(Long userId, Long friendId, Friend friend) {
        ...
        List<IMTerminalType> terminals = imClient.getOnlineTerminal(friendId);
        FriendVO vo = convert(friend, terminals);
        msgInfo.setContent(JSON.toJSONString(vo));
        IMPrivateMessage<PrivateMessageVO> sendMessage = new IMPrivateMessage<>();
    ...
    void sendTextMessage(Long sendId, Long recvId, String content) {
        ...
        PrivateMessageVO messageInfo = BeanUtils.copyProperties(msg, PrivateMessageVO.class);
        IMPrivateMessage<PrivateMessageVO> sendMessage = new IMPrivateMessage<>();
        sendMessage.setSender(new IMUserInfo(sendId, IMTerminalType.UNKNOW.code()));
        sendMessage.setRecvId(recvId);
        sendMessage.setSendToSelf(false);
        sendMessage.setData(messageInfo);
        imClient.sendPrivateMessage(sendMessage);
        // 推给自己
        sendMessage.setRecvId(sendId);
        imClient.sendPrivateMessage(sendMessage);
    }
