...
let jPreview = {
    ...
    startPreviw() {
        ...
        let pdfExt = ['pdf'];
        let txtExt = ['txt', 'xml', 'json', 'html', 'htm', 'css', 'js', 'java', 'py', 'cpp', 'c', 'md', 'markdown', 'ini', 'cfg', 'conf', 'log', 'sql', 'yml', 'yaml', 'bat', 'sh', 'rtf'];
    ...
    pdfView(url) {
        setTimeout(()=>{
             $("body").html("<iframe src='" + this.config.staticPath + "/pdfjs/web/viewer.html?file=" + url + "' style='width:100%;height:100vh;border:none;display:block'></iframe>");
        },10)
    },
    imgView(url) {
